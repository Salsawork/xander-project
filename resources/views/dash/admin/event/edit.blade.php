...
@php
     ...
     *
     * Yang disimpan di DB: HANYA nama file (contoh: "20241107123456-event-keren.jpg")
     * atau path lama (akan dinormalisasi dengan basename()).
     */

    $feEventBase = rtrim(asset('images/event'), '/') . '/';
    ...
    // Nilai tampilan (format ribuan)
    $fmt = fn($n) => $n ? number_format((int)$n, 0, ',', '.') : '';

    /**
     * Normalisasi image_url:
     * - Jika http/https → pakai apa adanya
     * - Selain itu → ambil basename lalu gabung ke $feEventBase
     */
    $currentImg = null;
    if (!empty($event->image_url)) {
        ...
        if (preg_match('/^https?:\/\//i', $raw)) {
            ...
            if ($filename !== '' && $filename !== '.' && $filename !== '/') {
                $currentImg = $feEventBase . $filename;
            }
        }
    }
@endphp
  ...
  <div class="flex flex-1 min-h-0">
    ...
    <main class="flex-1 overflow-y-auto min-w-0 mb-8 scroll-safe">
      ...
      <div class="mt-20 px-4 sm:px-8 max-w-7xl mx-auto w-full">
        ...
        <div class="flex items-center justify-between gap-3 mb-6">
          <div>
            ...
            <p class="text-sm text-gray-400 mt-1">
              Perbarui informasi event. Gambar dibaca dari <code>/images/event/</code>.
            </p>
          </div>
        </div>
        ...
        {{-- Alerts --}}
        @if (session('success'))
          <div class="mb-4 rounded-lg border border-green-700 bg-green-900/30 px-4 py-3 text-sm">
            {{ session('success') }}
          </div>
        @endif
        @if (session('error'))
        ...
        >
          ...
          <section class="card p-5 sm:p-7">
            ...
            <div class="mt-5 space-y-5">
              ...
              <div>
                ...
                <textarea
                  class="form-input whitespace-pre-wrap break-words"
                  id="description"
                  name="description"
                  rows="4"
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label class="form-label" for="price_ticket_view">Harga Tiket Penonton (Rp)</label>
                  <input
                    class="form-input"
                    id="price_ticket_view"
                    type="text"
                    inputmode="numeric"
                    autocomplete="off"
                ...
                <div>
                  <label class="form-label" for="stock">Stok Tiket Penonton</label>
                  <input
                    class="form-input"
                    id="stock"
                    name="stock"
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="Jumlah slot pemain"
                  >
              ...
              {{-- Status --}}
              <div>
                <label class="form-label" for="status">Status Event</label>
                <select
                  id="status"
                  name="status"
                  class="form-input"
                >
                  @foreach (['Upcoming','Ongoing','Ended'] as $status)
                    <option
                      value="{{ $status }}"
          ...
          <section class="flex flex-col gap-5">
              ...
              <div class="mt-5 space-y-4">
                <div>
                  <label class="form-label" for="image_url">
                    Upload Gambar (disimpan sebagai filename di <code>/images/event</code>)
                  </label>
                  <input
                    class="form-input"
                    id="image_url"
                ...
                @if($currentImg)
                  <div>
                    <p class="text-xs text-gray-400 mb-2">
                      Gambar saat ini (dibaca dari <code>/images/event/{{ basename($currentImg) }}</code>):
                    </p>
                    <img
                      src="{{ $currentImg }}"
                      alt="{{ $event->name }}"
            ...
            <div class="flex items-center justify-end gap-3">
              <a href="{{ route('admin.event.index') }}" class="btn btn-outline-danger">
                Batal
              </a>
