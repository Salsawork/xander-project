{{-- resources/views/admin/order/product.blade.php --}}
@extends('app')
...
@php
    ...
    $__proofCdnBase = rtrim(asset('images/payment_proof'), '/') . '/';

    if (!function_exists('proof_cdn_url_product')) {
        /**
         * Cari URL bukti transfer untuk 1 order product.
         *
         * Urutan:
         * 1) Scan folder /images/payment_proof pakai pola:
         *      *{order_number}*.ext atau *{id}*.ext  (jpg,jpeg,png,webp,pdf)
         *    Ambil file terbaru.
         * 2) Jika tidak ada:
         ...
         *          - proof_file
         *      lalu paksa ke:
         *          https://xanderbilliard.site/images/payment_proof/{basename}
         */
        function proof_cdn_url_product($order, string $dir, string $cdnBase): ?string {
        ...
        <main class="flex-1 overflow-y-auto min-w-0 mb-8 scroll-safe">
                ...
                <div class="relative">
                    ...
                    <div
                        ...
                        <div class="flex-shrink-0 snap-center min-w-[100px] sm:flex-1 text-center">
                            <div class="text-2xl sm:text-3xl font-extrabold leading-none">{{ $pendingCount ?? 0 }}</div>
                            <div class="font-semibold mt-1 text-sm sm:text-base">Pending</div>
                        </div>
                        <div class="border-l border-gray-600 flex-shrink-0"></div>
                        <div class="flex-shrink-0 snap-center min-w-[100px] sm:flex-1 text-center">
                            <div class="text-2xl sm:text-3xl font-extrabold leading-none">{{ $processingCount ?? 0 }}</div>
                            <div class="font-semibold mt-1 text-sm sm:text-base">Processing</div>
                        </div>
                        <div class="border-l border-gray-600 flex-shrink-0"></div>
                        <div class="flex-shrink-0 snap-center min-w-[100px] sm:flex-1 text-center">
                            <div class="text-2xl sm:text-3xl font-extrabold leading-none">{{ $shippedCount ?? 0 }}</div>
                            <div class="font-semibold mt-1 text-sm sm:text-base">Shipped</div>
                        </div>
                        <div class="border-l border-gray-600 flex-shrink-0"></div>
                        <div class="flex-shrink-0 snap-center min-w-[100px] sm:flex-1 text-center">
                            <div class="text-2xl sm:text-3xl font-extrabold leading-none">{{ $deliveredCount ?? 0 }}</div>
                            <div class="font-semibold mt-1 text-sm sm:text-base">Delivered</div>
                        </div>
                        <div class="border-l border-gray-600 flex-shrink-0"></div>
                        <div class="flex-shrink-0 snap-center min-w-[100px] sm:flex-1 text-center">
                            <div class="text-2xl sm:text-3xl font-extrabold leading-none">{{ $cancelledCount ?? 0 }}</div>
            ...
            <section class="flex flex-col sm:flex-row sm:items-center sm:justify-between mx-4 sm:mx-8 mb-4 gap-4">
                <input
                    ...
                    onchange="
                        window.location.href='{{ route('order.index.product') }}'
                ...
                <div class="flex flex-col sm:flex-row gap-2 text-xs text-gray-600 select-none">
                    <div class="relative">
                        ...
                        >
                            ...
                            @foreach (['pending','processing','packed','shipped','delivered','cancelled','returned'] as $opt)
                ...
                <table class="w-full text-left text-gray-300 border-collapse">
                    ...
                    <tbody class="text-sm font-normal">
                        @forelse($orders as $order)
                            @php
                                $proofUrl = proof_cdn_url_product($order, $__proofDir, $__proofCdnBase);
                            ...
                            <tr class="border-b border-gray-700">
                                ...
                                <td class="px-4 py-3">
                                    <span class="{{ $payStatusClass[$order->payment_status] ?? 'bg-gray-500 text-white' }} px-3 py-1 rounded-full inline-block min-w-[80px] text-center">
                                ...
                                <td class="px-4 py-3">
                                    <span class="{{ $shipStatusClass[$order->delivery_status] ?? 'bg-gray-500 text-white' }} px-3 py-1 rounded-full inline-block min-w-[80px] text-center">
                                ...
                                <td class="px-4 py-3 flex gap-4 text-gray-500">
                                    ...
                                    <div class="relative" x-data="{ open: false }">
                                        <button @click="open = !open"
                                                aria-label="Update delivery status order {{ $order->id }}"
                                        ...
                                        <div x-show="open" x-cloak @click.away="open = false"
                                            ...
                                            <div class="py-1">
                                                @foreach (['pending','processing','packed','shipped','delivered','cancelled'] as $st)
                                                    <a href="{{ route('admin.orders.update-status', ['order'=>$order->id,'status'=>$st]) }}"
            ...
            <section class="lg:hidden mx-4 space-y-4">
                @forelse($orders as $order)
                    @php
                        $proofUrlMobile = proof_cdn_url_product($order, $__proofDir, $__proofCdnBase);
                    ...
                    @endphp
                    <div class="bg-[#292929] rounded-lg p-4" x-data="{ open:false, openPayment:false }">
                        <div class="flex justify-between items-start mb-3">
                            ...
                            <div class="flex flex-col items-end gap-1">
                                ...
                                <span class="{{ $deliveryStatusClass[$order->delivery_status] ?? 'bg-gray-500 text-white' }} px-3 py-1 rounded-full text-xs">
                                    {{ ucfirst($order->delivery_status) }}
                                </span>
                        ...
                        <div class="flex gap-3 mt-4 pt-3 border-t border-gray-700 justify-end text-xs">
                            <a href="{{ route('order.detail.index', $order->id) }}"
                               aria-label="View order {{ $order->id }}"
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Konfirmasi update delivery status
    ...
    // Konfirmasi update payment status
