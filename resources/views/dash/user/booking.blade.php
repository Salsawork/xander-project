{{-- resources/views/dash/user/booking.blade.php --}}
@extends('app')
@section('title', 'User Dashboard - Booking')
...
@php
   ...
   *
   * Lokasi fisik upload:
   *   /home/xanderbilliard.site/public_html/images/venue/{filename}
   *
   * URL publik di Blade:
  ...
  // Fallback file yang disiapkan di /public/images/venue
      ...
      <section class="flex-1 overflow-auto space-y-6 mt-16 mx-16 bg-neutral-900 scroll-inner">
        ...
        <nav class="flex space-x-6 text-gray-400 text-sm font-semibold mb-6 border-b border-gray-700">
          <a href="{{ route('booking.index') }}"
            ...
            <span>All</span>
            <span class="bg-gray-700 rounded text-xs font-normal px-2 py-[2px]">{{ $orderCount ?? 0 }}</span>
          </a>
              ...
              <ul class="divide-y divide-gray-600">
                @foreach($order->bookings as $booking)
                  @php
                    ...
                    // 1. Ambil sumber gambar mentah dari venue (image / images)
                    $raw = null;
                    if ($venue) {
                        if (!empty($venue->image)) {
                            $raw = $venue->image;
                        }
                        if (!$raw && !empty($venue->images)) {
                            $imgs = $venue->images;
                            if (is_array($imgs)) {
                                $raw = $imgs[0] ?? null;
                    ...
                    // 2. Normalize -> selalu ambil basename dari berbagai bentuk path:
                    //    - full URL
                    //    - /images/venue/foo.jpg
                    //    - images/venue/foo.jpg
                    //    - path server /home/.../images/venue/foo.jpg
                    $basename = null;
                    if ($raw) {
                    ...
                    // 3. Bangun URL final: selalu dari /images/venue/{basename}
                    if ($basename && $basename !== '/' && $basename !== '.') {
                        $imgUrl = $venueBaseUrl . $basename;
                    } else {
                        // fallback round-robin
                        $idx = count($venueFallbacks) ? ($loop->index % count($venueFallbacks)) : 0;
                        $fallbackName = $venueFallbacks[$idx] ?? $venueFallbacks[0] ?? null;
                        $imgUrl = $fallbackName ? ($venueBaseUrl . $fallbackName) : $venueBaseUrl;
                    }

                    // 4. Fallback onerror → fallback pertama di /images/venue
                    $firstFallback = $venueFallbacks[0] ?? null;
                    $fallbackUrl = $firstFallback ? ($venueBaseUrl . $firstFallback) : $venueBaseUrl;
                  @endphp
