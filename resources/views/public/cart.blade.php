            ...
            <ul class="space-y-4">
                @forelse ($carts as $cart)
                    <li class="flex items-center space-x-4">
                        ...
                        <img alt="{{ $cart['name'] }}" class="w-20 h-20 rounded-md object-cover flex-shrink-0"
                            height="80" src="{{ $cart['image'] ?? 'https://placehold.co/400x400?text=No+Image' }}"
                            width="80" />
                        <div class="flex-1">
                        ...
                        <form action="{{ route('cart.del') }}" method="POST" class="delete-form">
                            @csrf
                ...
                @forelse ($sparrings ?? [] as $index => $sparring)
                    <li class="flex items-center space-x-4">
                        ...
                        <img alt="{{ $sparring['name'] }}" class="w-20 h-20 rounded-md object-cover flex-shrink-0"
                            height="80" src="{{ $sparring['image'] ? asset('images/athlete/' . $sparring['image']) : 'https://placehold.co/400x400?text=No+Image' }}"
                            width="80" />
                        <div class="flex-1">
                            <p class="font-bold text-white text-base leading-tight">
                            ...
                            <input type="hidden" id="price-sparring-{{ $sparring['schedule_id'] }}" value="{{ $sparring['price'] }}">
                            <p class="text-white text-sm mt-1">
                                Rp. {{ number_format($sparring['price'], 0, ',', '.') }},-
                            </p>
                        </div>
                        <form action="{{ route('sparring.removeFromCart') }}" method="POST" class="delete-form">
                            @csrf
                            @method('DELETE')
