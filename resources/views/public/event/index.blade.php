    ...
    <main class="min-h-screen bg-neutral-900 text-white">
        ...
        <section class="relative isolate">
            <div class="relative overflow-hidden vh-section">
                <img src="{{ asset('images/event/bg-event-1.png') }}" alt="Events Hero"
                    class="hero-img absolute inset-0 h-full w-full object-cover object-center" loading="eager" decoding="async"
                ...
                <div class="relative z-10 h-full flex items-center">
                    <div class="w-full max-w-7xl mx-auto px-6 md:px-20">
                        ...
                        <h1
                            class="hero-title text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold uppercase leading-tight">
        ...
        <section class="max-w-7xl mx-auto px-4 md:px-12 lg:px-16 py-10">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('events.list', ['status' => 'upcoming']) }}"
                    class="text-sm text-gray-300 hover:text-white">view all</a>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                @forelse (($upcomingEvents ?? collect()) as $ev)
                    ...
                    <a href="{{ $link }}" class="group">
                        <div class="ev-card">
                            <div class="ev-thumb h-40 md:h-44 relative">
                                @if ($img)
                                    <img src="{{ $img }}" alt="{{ $ev->name }}" loading="lazy" decoding="async"
                                        onerror="this.style.display='none'">
                                @else
                                    <div class="w-full h-full grid place-items-center text-gray-400"><i
                                            class="far fa-image text-3xl"></i></div>
                                @endif
                            </div>
                            <div class="p-4">
                                ...
                                <ul class="space-y-2 text-[13px] text-gray-300">
                                    <li class="flex items-center gap-2"><i
                                            class="far fa-clock w-4 text-gray-400"></i><span>{{ $range }}</span></li>
                                    <li class="flex items-center gap-2"><i
                                            class="fas fa-map-marker-alt w-4 text-gray-400"></i><span
                                            class="line-clamp-1">{{ $ev->location }}</span></li>
                                    <li class="flex items-center gap-2"><i
                                            class="fas fa-gamepad w-4 text-gray-400"></i><span>{{ $ev->game_types }}</span>
                                    </li>
                                </ul>
                            </div>
                        </div>
        ...
        <section class="max-w-7xl mx-auto px-4 md:px-12 lg:px-16 pb-14">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('events.list', ['status' => 'ongoing']) }}"
                    class="text-sm text-gray-300 hover:text-white">view all</a>
            </div>
                    ...
                    <a href="{{ $link }}" class="group">
                        <div class="ev-card">
                            ...
                            <div class="p-4">
                                ...
                                <ul class="space-y-2 text-[13px] text-gray-300">
                                    <li class="flex items-center gap-2"><i
                                            class="far fa-clock w-4 text-gray-400"></i><span>{{ $range }}</span>
                                    </li>
                                    <li class="flex items-center gap-2"><i
                                            class="fas fa-map-marker-alt w-4 text-gray-400"></i><span
                                            class="line-clamp-1">{{ $ev->location }}</span></li>
